...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.array([[1, 0] * n, [0, 1] * n] * n)    # Credit to Juan: Elegant solution.
    # ------ Place code above here /\ /\ /\ ------
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(np.random.randint(0,n), index=pd.date_range("1/1/2010", periods=1000))
    ts = s.cumsum()
    ts.plot()
    # ------ Place code above here /\ /\ /\ ------
    return s
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    #df = pd.DataFrame(words, columns=['Length'])
    data = pd.DataFrame(words, columns=['words'])
    df = data['words'].str.len()
    # ------ Place code above here /\ /\ /\ ------
    return df
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.read_csv('https://tinyurl.com/y63q7okz').iloc[:, 0:3:2]
